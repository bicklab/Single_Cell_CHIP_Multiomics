...
make_volcano_plots = function(cell_type, sample_genotype, output_dir, table_dir, stim_status, x_limit = 1.5, y_limit = 300) {
  plots = list()
  cell_type = gsub(" ", "_", cell_type)
  folder = "tables/supplemental/"
  input_filename = list.files(folder, pattern = paste0("patient_", sample_genotype, "_", cell_type, ".*_", stim_status))
  ...
  filename = paste0(output_dir, "patient_", sample_genotype, "_", cell_type, "_", stim_status)
  save_plot(filename, plot, legend, width = 9)
  write_tsv(data, paste0(table_dir, "patient_", sample_genotype, "_", cell_type, "_", stim_status, ".tsv"))
...
Idents(tet2_patient_singlet_data) = "STIM"
stim_data = subset(tet2_patient_singlet_data, idents = "STIM")
veh_data = subset(tet2_patient_singlet_data, idents = "VEH")

# DATA HAS ALREADY BEEN NORMALIZED AND SCALED SO SKIPPING THAT STEP
...
Idents(dnmt3a_patient_singlet_data) = "STIM"
stim_data = subset(dnmt3a_patient_singlet_data, idents = "STIM")
veh_data = subset(dnmt3a_patient_singlet_data, idents = "VEH")
...
for (cell_type in cell_types) {
  perform_stats(veh_data, cell_type, "DNMT3A", "unstim")
  perform_stats(stim_data, cell_type, "DNMT3A", "stim")
}

for (cell_type in cell_types) {
  make_volcano_plots(cell_type = cell_type, sample_genotype = "DNMT3A", output_dir = "figures/figure_3/", table_dir = "tables/figure_3/", stim_status = "unstim", y_limit = 20)
  make_volcano_plots(cell_type = cell_type, sample_genotype = "DNMT3A", output_dir = "figures/figure_4/", table_dir = "tables/figure_4/", stim_status = "stim", y_limit = 20)
}
