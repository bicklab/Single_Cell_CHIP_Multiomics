...
find_pathways = function(sample_genotype, cell_type, stim_status, use_cutoff = TRUE) {
  ...
  tryCatch({
    ...
    title = paste(stim_status, sample_genotype, cell_type_for_title)
    ...
    # save plots
    filename = paste0(output_dir, "dotplot_", sample_genotype, "_", cell_type_name, ifelse(use_cutoff, "", "_no_cutoff"), "_", stim_status)
    ...
    filename_all = paste0(output_dir_all, "dotplot_", sample_genotype, "_", cell_type_name, ifelse(use_cutoff, "", "_no_cutoff"), "_", stim_status, "_all")
...
```{r}
input_filename = "CD14_Monos_DNMT3A_Mutant_vs_Wildtype.tsv"
title = "DNMT3A CD14 Monos Mutant/Wildtype"

input_filename = "CD14_Monos_TET2_Mutant_vs_Wildtype.tsv"
title = "TET2 CD14 Monos Mutant/Wildtype"
```
...
plot_volcano = function(sample_genotype, cell_type, stim_status) {
  ...
  (volcano_plot = ggplot(exp_data %>% arrange(desc(color)), aes(x = log2FoldChange, y = -log10(padj), color = color)) +
    ...
    ggtitle(paste(stim_status, sample_genotype, cell_type_for_title)) +
   #  # negative labels
...
get_color = function(description) { 
  switch(description,
       "B signal" = "green",
       "inflammatory response" = "white",
       "ER" = "red")
}

get_color("inflammatory response")
```
...
title = paste("VEH", "TET2", "Monocytes")
...
title = paste("VEH", "DNMT3A", "Monocytes")
