...
make_metacells_fig = function(path_to_data, path_to_labels, title, output_path) {
  data = ReadH5AD(path_to_data)
  data_labels = read_csv(path_to_labels)

  relevant_metadata = diet_seurat_trimmed@meta.data %>%
    rownames_to_column("cell_ID") %>%
    dplyr::select(cell_ID, cell_type, GROUP) %>%
    inner_join(data_labels, by = c(cell_ID = "...1")) %>%
    filter(metacell != -1)
  ...
  data@meta.data$metacell = rownames(data@meta.data) %>% as.numeric()
  data@meta.data = data@meta.data %>%
    inner_join(celltypes_for_metacells)
  
  (metacells_plot = ggplot(data@meta.data, aes(x = umap_x, y = umap_y, color = cell_type)) +
    geom_point() +
    theme_bw() +
  ...
  save_plot(metacells_plot, output_path, width = 6)
}


make_metacells_fig(path_to_data = "h5ad_files/tet2_stim_for_seurat.h5ad", path_to_labels = "metacells/tet2_stim_metacells.csv", title = "TET2 +IL6 Metacells", output_path = "figures/metacells/tet2_stim_metacells")
make_metacells_fig(path_to_data = "h5ad_files/tet2_veh_for_seurat.h5ad", path_to_labels = "metacells/tet2_veh_metacells.csv", title = "TET2 Metacells", output_path = "figures/metacells/tet2_veh_metacells")
make_metacells_fig(path_to_data = "h5ad_files/dnmt3a_veh_for_seurat.h5ad", path_to_labels = "metacells/dnmt3a_veh_metacells.csv", title = "DNMT3A Metacells", output_path = "figures/metacells/dnmt3a_veh_metacells")
make_metacells_fig(path_to_data = "h5ad_files/none_veh_for_seurat.h5ad", path_to_labels = "metacells/none_veh_metacells.csv", title = "Control Metacells", output_path = "figures/metacells/control_veh_metacells")
