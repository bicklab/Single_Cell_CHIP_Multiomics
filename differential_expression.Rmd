...
# DIDNT USE. Install IDEAS package.
```{r}
library("devtools");
install_github("Sun-lab/ideas")
library(ideas)
```
# DIDNT USE. Code for IDEAS analysis. 
```{r}
...
# DIDNT USE. Code to run in a WDL.
```{r}
...
## DIDNT USE. Pseudobulk method based on https://github.com/elolab/multisubjectDSanalysis
# DIDNT USE. Pseudobulk functions.
```{r}
...
# DIDNT USE. Run pseudobulk functions.
```{r}
library(muscat)
library(ROTS)
...
# USED THIS ONE BUT IN METACELLS2 MARKDOWN. Pseudobulk with Sam's code.
```{r}
###############################
...
Pseudobulk <- function(agg.sce, label1, label2, exclude, shrinkage = T, cell_type) {
    ...
    resOrderedDFwald <- as.data.frame(resOrderedwald) %>%
      rownames_to_column("gene")
    write_tsv(resOrderedDFwald, paste0("tables/meta_pseudobulk_one_per_patient/", cell_type, "_", gsub(" ", "_", label1), "_vs_", gsub(" ", "_", label2), ".tsv"))
...
# Try DE for subclusters of monocytes
```{r}
...
subclustered_seurat = FindSubCluster(seurat_df_harmony_keep_0_75, cluster = "Classical Monocytes", graph.name = "RNA_snn", resolution = 0.03)

```
```{r}
library(scProportionTest)

Idents(subclustered_seurat) = "sub.cluster"
subclustered_seurat_monocytes = subset(subclustered_seurat, ident = c("Classical Monocytes_0", "Classical Monocytes_1", "Classical Monocytes_2"))


prop_test = sc_utils(subclustered_seurat_monocytes)
prop_test = permutation_test(
  prop_test, cluster_identity = "sub.cluster",
  sample_1 = "control", sample_2 = "chip",
  sample_identity = "CHIP"
)
View(prop_test@results$permutation)
(prop_plot = permutation_plot(prop_test))
```

```{r}
cell_type = "Classical Monocytes_2"
Idents(subclustered_seurat) = "sub.cluster"
subclustered_seurat_monocytes = subset(subclustered_seurat, ident = cell_type)

agg_cell_type_data <- Aggregate(subclustered_seurat_monocytes)
  cell_type = gsub(" ", "_", gsub("+", "", cell_type, fixed = TRUE))
  tet_veh <- Pseudobulk(agg_cell_type_data, exclude = c(), label1 = "TET2 VEH", label2 = "none VEH", cell_type = cell_type)
  tet_stim <- Pseudobulk(agg_cell_type_data, exclude = c(), label1 = "TET2 STIM", label2 = "none STIM", cell_type = cell_type)
  dnmt3a_veh <- Pseudobulk(agg_cell_type_data, exclude = c(), label1 = "DNMT3A VEH", label2 = "none VEH", cell_type = cell_type)
...
Idents(subclustered_seurat) = "customclassif"
subclustered_seurat_monocytes = subset(subclustered_seurat, ident = "Classical Monocytes")
pbmc_small_sce <- as.SingleCellExperiment(subclustered_seurat)
pbmc_small_milo <- Milo(pbmc_small_sce)
traj_milo <- buildGraph(pbmc_small_milo)
